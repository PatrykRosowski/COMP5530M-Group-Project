  ...
  const handleFileChange = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Transform GeoJSON coordinates [lng, lat] to Leaflet [lat, lng]
        let coords = [];
        if (data.geometry && data.geometry.coordinates) {
          coords = data.geometry.coordinates.map((coord) => [
            coord[1],
            coord[0],
          ]);
        ...
        const newRoute = {
          id: data.id || `L${routes.length + 1}-${Date.now()}`,
        ...
        if (newRoute.coordinates.length > 0) {
          ...
          alert("Selected file does not contain valid route coordinates.");
        }
      } catch (err) {
        console.error("Error parsing route file:", err);
        alert(
          "Failed to load route file. Please ensure it is a valid JSON produced by the Route Builder.",
        );
      }
    };
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`text-sm font-medium transition-colors duration-200 ${route.visible ? "text-slate-700" : "text-slate-400"}`}
                    >
                      {route.name}
                    </span>
                    <RouteTag
